...
async function GET(req) {
  ...
  if (!tokens.refresh_token) {
    return NextResponse.json({
      error: "No refresh_token returned. Revoke app access on Google Account lalu connect ulang."
    }, { status: 400 });
  }
  ...
  const redirectUrl = new URL("/accounts", process.env.BASE_URL || url.origin);
