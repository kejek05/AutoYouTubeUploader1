...
async function POST(req) {
  ...
  for (const a of accounts) {
    const rows = db.prepare("SELECT id, video_id FROM uploads WHERE user_id=? AND youtube_account_id=? AND video_id IS NOT NULL ORDER BY id DESC LIMIT 50").all(user.id, a.id);
