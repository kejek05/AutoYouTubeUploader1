...
export default function GalleryVideos() {
  ...
  async function load() {
    const r = await fetch("/api/files/list?kind=video");
  ...
  async function doUpload() {
    ...
    setProgress(0);
    try {
      const j = await xhrUpload({ file: selected, kind: "video", onProgress: setProgress });
      setMsg(`OK: ${j.path}`);
      setSelected(null);
      await load();
    } catch (e) {
  ...
  return (
    <div className="card" style={{ marginTop: 16 }}>
      <div style={{display:"flex",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        <div>
          <h2 style={{ marginTop: 0 }}>Gallery • Videos</h2>
          <div className="small">Upload video ke server, lalu copy path untuk dipakai di Uploads.</div>
        </div>
        <div style={{display:"flex",gap:10,alignItems:"end",flexWrap:"wrap"}}>
          <a className="btn" href="/gallery/thumbnails">Ke Thumbnails</a>
          <a className="btn" href="/uploads">Ke Uploads</a>
        </div>
      </div>

      <div className="grid" style={{ marginTop: 12 }}>
        <input className="input" type="file" accept="video/*" onChange={(e)=>setSelected(e.target.files?.[0] || null)} />
        <button className="btn btn2" type="button" onClick={doUpload}>Upload Video</button>
        ...
        <div style={{ overflowX: "auto" }}>
          <table>
            ...
            <tbody>
              ...
              {!files.length && <tr><td colSpan="4" className="small">Belum ada video.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
